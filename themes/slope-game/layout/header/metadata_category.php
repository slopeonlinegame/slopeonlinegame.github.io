...
$title = "Play " . $meta_title . ' on ' . \helper\options::options_by_key_type('site_name');
$metadata = json_decode($category->metadata);
$meta_description = html_entity_decode($metadata->description);
if (empty($meta_description)) {
    $list_games = \helper\game::get_paging(1, 5, $keywords, $type, $display, $is_hot, $is_new, $field_order, $order_type, $category->id);
    $count = count($list_games);
    $meta_description = 'Come and enjoy the best ' . $category->name . ':';
