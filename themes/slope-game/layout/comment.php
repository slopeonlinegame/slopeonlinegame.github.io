...
<div class="comment-company">
    <div id="comments_area" data-url="<?php echo $url ?>">
        <div class="comment_loading"></div>
        <?php if ($count_comment != null && $count_comment > 0) : ?>
            <div class="comment-header">
                <div class="comment-count" style="font-size: 18px;">
                    Comments<span id="comment_count" data-count="<?php echo $count_comment ?>">(<?php echo $count_comment ?>)</span></label>
                </div>
        ...
        <div id="list_comment" data-cached-html="<?php echo $ads_cached_html ?>">
            <?php echo $ads_cached_html ? \helper\themes::get_layout('comment_paging', ['url' => $url]) : ''; ?>
        </div>
        ...
        <div class="make-comment">
            <form id="comment_form" class="form-group">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <input id="comment_author" name="comment_author" class="form-control2" type="text" value="" size="30" maxlength="50" placeholder="Name" autocomplete="off">
                    </div>
                    <div class="col-md-6">
                        <input id="comment_email" name="comment_email" type="input" value="" size="30" class="form-control2" maxlength="100" placeholder="Email" autocomplete="off">
                    </div>
                    <div class="col-md-12">
                        <textarea id="comment_content" name="comment_content" class="form-control2" cols="45" rows="3" maxlength="65525" placeholder="Content"></textarea>
                    </div>
        ...
        <script>
            ads_cached_html = "<?php echo $ads_cached_html ?>";
        </script>
        <!-- when user cmt -->
        <script>
                ...
                submitHandler: function() {
                    ...
                    if (ads_cached_html) {
                        var queryParams = {
                            ...
                            website: website,
                            parent_id: parent_id,
                            related_id: parseInt("0"),
                            related_url: "<?php echo $url; ?>"
                        };
                        var url_make_cmt = question_ajax + '?' + jQuery.param(queryParams); // Build URL with query string
                        var data_make_cmt = '';
                        var type_make_cmt = 'GET';
                    } else {
                        var metadataload = {};
                        ...
                        var url_make_cmt = question_ajax;
                        var data_make_cmt = metadataload;
                        var type_make_cmt = 'POST';
                    }
                        ...
                        success: function(data) {
                            ...
                            if (data != '') {
                                ...
                                if (result.result === true) {
                                    ...
                                    jQuery("#comment_form input[type='submit']").prop("disabled", true);
                                    setTimeout(function() {
                                        jQuery("#comment_form input[type='submit']").prop("disabled", false);
                                    }, 45000);
                                }
                            }
                        }
        ...
        <script>
            ...
            function comment_vote(comment_id, vote) {
                ...
                if (ads_cached_html) {
                    ...
                    var url_vote_cmt = comment_comment_voteajax + '?' + jQuery.param(queryParams); // Build URL with query string
                    var data_vote_cmt = '';
                    var type_vote_cmt = 'GET';
                } else {
                    var metadataload = {};
                    metadataload.comment_id = comment_id;
                    metadataload.vote = vote;
                    var url_vote_cmt = comment_comment_voteajax;
                    var data_vote_cmt = metadataload;
                    var type_vote_cmt = 'POST';
                }
                jQuery.ajax({
                    url: url_vote_cmt,
                    data: data_vote_cmt,
        ...
        <script>
            ...
            // load_comment (load website) + show more cmt (limit first 5 cmt) + click btn: Sort by
            function load_comment(page, limit, sort, url, main_contain_id, refresh) {
                ...
                if (ads_cached_html) {
                    ...
                    var url_cmt = mainposturl + '?' + jQuery.param(queryParams); // Build URL with query string
                    var data_cmt = '';
                    var type_cmt = 'GET';
                } else {
                    var metadataload = {};
                    ...
                    var url_cmt = mainposturl;
                    var data_cmt = metadataload;
                    var type_cmt = 'POST';
                }
                jQuery.ajax({
                    url: url_cmt,
            ...
            // check not ads_cached_html ==> action function load_comment
            if (!ads_cached_html) {
...
<style>
    ...
    .btn-primary:hover {
        ...
        background-color: #4b80c9;
        border-color: #4b80c9;
        background: linear-gradient(65deg, #2973da 0%, #0c50b8 100%);
    }
